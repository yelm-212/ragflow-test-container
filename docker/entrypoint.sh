...
function patch_auth_header_compat() {
    "$PY" - <<'PY'
from pathlib import Path

path = Path("/ragflow/api/apps/__init__.py")
old = '''def _load_user():
...
new = '''def _load_user():
    ...
    def _resolve_user(candidate):
        user = UserService.query(access_token=candidate, status=StatusEnum.VALID.value)
        if user:
            return user[0]
        user = UserService.query(id=candidate, status=StatusEnum.VALID.value)
        if user:
            return user[0]
        return None
...
if old in text:
    ...
    print("Patched auth header compatibility in /ragflow/api/apps/__init__.py")
else:
    print("Auth header compatibility patch already applied or source changed")
PY
}
